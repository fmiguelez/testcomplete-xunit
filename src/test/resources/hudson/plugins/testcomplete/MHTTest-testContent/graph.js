var c_graphFlashMovie = "MSLine.swf";

var m_graph_uniqueID = 0;

function graph_setActive(element)
{
	graph_clear(element);
	element.innerHTML = "";

	graph_CreateRootFrame(element);
	graph_CreateGraph(element);
}

function graph_CreateRootFrame(element)
{
	element.schemas = new Array("aqds:graph");
	...
	element.doResize = function(_Width, _Height)
	{
		...
		this.abscissSelector.style.width = ChartWidth - 20;
		this.ordinateSelector.style.width = ChartWidth - 20;
...
function graph_clear(element)
{
	if (element.m_dataTable)
	{
		table_clear(element.m_dataTable);
		graph_clear(element.m_dataTable);
	}
	if (element.m_graph) element.m_graph.m_parent = null;
	if (element.abscissSelector) element.abscissSelector.topControl = null;
	if (element.ordinateSelector) element.ordinateSelector.topControl = null;

	element.detailsDiv = null;
	element.m_dataTable = null;
	element.m_rootElement = null;
	element.m_graph = null;
	element.m_graph_div = null;
	element.m_graph_obj = null;
	element.m_graphPanel = null;
	element.abscissSelector = null;
	element.ordinateSelector = null;
}

function graph_CreateGraph(element)
{
	...
	m_graph_uniqueID ++;
	element.m_graph_id = "graph_" + m_graph_uniqueID;

	element.m_graph_obj = new FusionCharts(c_graphFlashMovie, element.m_graph_id, ChartWidth, ChartHeight, false, false, "white");
	...
	oAbsciss.style.width = ChartWidth - 20;
	oAbsciss.onchange = graph_abscissChanged;
	...
	oOrdinate.style.width = ChartWidth - 20;
	oOrdinate.onchange = graph_ordinateChanged;
	...
	for (var i = 0; i < element.table.columns.length; i++)
	{
		...
		if (oColumn.absciss && oColumn.absciss == true)
		{
			...
			if (oColumn.defaultAbsciss == true)
			{
	...
	graph_synchronizeOrdinate(element);

	if (isOpera) graph_updateAllTables(element);

	graph_regenerateGraph(element);

}

function graph_updateAllTables(element)
{
	var dt = element.m_dataTable;
	if (!dt) return;

	if (dt.m_NestedObject)
		graph_updateAllTables(dt.m_NestedObject);
...
function graph_synchronizeOrdinate(element)
{
	...
	var combinations = selectedColumn.combinations == null ? "": selectedColumn.combinations;
	...
	for(var i = 0; i < columnNames.length; i++)
	{
		...
		for(var j = 0; j < element.table.columns.length; j++)
		{
			...
			if (oColumn.name == columnName)
			{
				...
				if (oColumn.defaultOrdinate == true)
				{
...
function graph_regenerateGraph(element)
{
	...
	{
		var xmlValue = "<graph numdivlines='2' numVDivLines='2' showAnchors='0' showValues='0' animation='0' rotateNames='1'" +
			"lineColor='#FF0000' lineThickness='3' bgColor='#FFFFFF' showBorder='0' showAlternateHGridColor='0' chartLeftMargin='3' chartRightMargin='8' chartTopMargin='8' chartBottomMargin='3'>\n";
		var xmlCategories = "<categories>\n";
		var xmlDataset = "<dataset >\n";
		...
		for (var i = 0; i < rows.length; i++)
		{
			...
			if (nodeValue == "")
				nodeValue = "No Value";
			var _showName = "";
			if (i != 0 && i % _skipCategories != 0) _showName = " showname='0'";
			xmlCategories += "<category name='" + nodeValue + "'" + _showName + "/>\n";
			...
			xmlDataset += "<set value='" + nodeValue2 + "'/>\n";
		}
		
		xmlValue += xmlCategories + "</categories>\n";
		xmlValue += xmlDataset + "</dataset>\n";
...
function graph_abscissChanged()
{
	graph_synchronizeOrdinate(this.topControl);
	graph_regenerateGraph(this.topControl);
}

function graph_ordinateChanged()
{
	graph_regenerateGraph(this.topControl);
}
